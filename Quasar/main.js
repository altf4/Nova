      ...
      if (rowcount[0].rows === 0) {
        ...
        NovaCommon.dbqCredentialsGetSalt.all(user, function cb(err, salt) {
          if (err || (salt[0] == undefined)) {
            ...
            NovaCommon.dbqCredentialsCheckLogin.all(user, NovaCommon.HashPassword(password, salt[0].salt), function selectCb(err, results) {
              if (err) {
                console.log("Database error: " + err);
              }
...
var logFile = fs.createWriteStream('./serverLog.log', {
  flags : 'a'
});
...
if (NovaCommon.config.ReadSetting('MASTER_UI_ENABLED') === '1') {
  ...
  client.on('connectFailed', function(error) {
    ...
    if (!reconnecting) {
      ...
      clearReconnect = setInterval(function() {
        console.log('attempting reconnect to wss://' + connected);
...
app.get('/novaState.csv', function(req, res) {
  exec('novacli get all csv > ' + NovaHomePath + "/state.csv", function(error, stdout, stderr) {
    if (error != null) {
...
app.get('/editUsers', function(req, res) {
  ...
  NovaCommon.dbqCredentialsGetUsers.all(function(err, results) {
    if (err) {
...
app.get('/', function(req, res) {
  NovaCommon.dbqFirstrunCount.all(function(err, results) {
    if (err) {
      RenderError(res, "Database Error: " + err);
      return;
    }
...
app.get('/about', function(req, res) {
  res.render('about.jade', {
    locals : {
      version : NovaCommon.config.GetVersionString()
    }
  });
});
...
app.post('/createNewUser', function(req, res) {
  ...
  NovaCommon.dbqCredentialsGetUser.all(userName, function selectCb(err, results, fields) {
    if (err) {
...
app.post('/createInitialUser', function(req, res) {
  ...
  NovaCommon.dbqCredentialsGetUser.all(userName, function selectCb(err, results) {
    if (err) {
      RenderError(res, "Database Error: " + err);
      return;
    }
    ...
    fs.writeFile(NovaHomePath + "/config/keys/quasarKey.pem", data, function(writeErrKey) {
      ...
      fs.readFile(req.files["cert"].path, function(readErrCert, certData) {
        fs.writeFile(NovaHomePath + "/config/keys/quasarCert.pem", certData, function(writeErrCert) {
          if (readErrKey != null) {
            RenderError(res, "Error when reading key file");
            return;
          }
          if (readErrCert != null) {
            RenderError(res, "Error when reading cert file");
            return;
          }
          if (writeErrKey != null) {
            RenderError(res, "Error when writing key file");
            return;
          }
          if (writeErrCert != null) {
            RenderError(res, "Error when writing cert file");
            return;
          }
...
app.get('/scripts', function(req, res) {
  ...
  for (var i = 0; i < scriptNames.length; i++) {
    ...
    namesAndPaths.push({
      ...
      path : script.GetPath(),
      configurable : script.GetIsConfigurable()
    });
  }
